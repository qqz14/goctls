...
import (
	...
	"github.com/qqz14/goctls/api/gogen/ent"
	"github.com/qqz14/goctls/util/format"

	apiformat "github.com/qqz14/goctls/api/format"
	"github.com/qqz14/goctls/api/gogen/proto"
	"github.com/qqz14/goctls/api/parser"
	apiutil "github.com/qqz14/goctls/api/util"
	"github.com/qqz14/goctls/config"
	"github.com/qqz14/goctls/pkg/golang"
...
var (
	...
	// VarStringResponsePackage describes the unified response package name (relative to <project>/api/internal/).
	// 为空则保持老的 handler 生成形态；非空则生成 response.Result(...) 单行封装。
	VarStringResponsePackage string
)
...
func GoCommand(_ *cobra.Command, _ []string) error {
	...
	genCtx := &GenContext{
		...
		ResponsePackage: VarStringResponsePackage,
	}
	if len(remote) > 0 {
...
type GenContext struct {
	...
	UseValidator  bool
	UseCoreRpc    bool
	// ResponsePackage 统一响应包名（相对于 <project>/api/internal/），空串为关闭。
