...
import (
	...
	"github.com/qqz14/goctls/extra/ent/importschema/mux"
...
var joinTableErr = errors.New("join tables must be inspected with ref tables - append `tables` flag")
...
func NewImport(opts ...ImportOption) (SchemaImporter, error) {
	...
	default:
		return nil, fmt.Errorf("unsupported dialect %q", i.driver.Dialect)
	}
	return si, err
}
...
func resolvePrimaryKey(field fieldFunc, table *schema.Table) (f ent.Field, err error) {
	if table.PrimaryKey == nil {
		return nil, fmt.Errorf("missing primary key (table: %v)", table.Name)
	}
	if len(table.PrimaryKey.Parts) != 1 {
		return nil, fmt.Errorf("invalid primary key, single part key must be present (table: %v, got: %v parts)", table.Name, len(table.PrimaryKey.Parts))
	}
